...
import { Moon, ArrowRight, MapPin, Users, Camera } from 'lucide-react';

gsap.registerPlugin(ScrollTrigger);

interface AncestralJourneySectionProps {
  className?: string;
}

const AncestralJourneySection = ({ className = '' }: AncestralJourneySectionProps) => {
  ...
  const heritageItems = [
    { icon: MapPin, text: 'Village mapping + archives' },
    { icon: Users, text: 'Local fixer + translator' },
    { icon: Camera, text: 'Private photo documentation' },
  ];
  ...
  return (
    ...
    >
      ...
      >
        <Moon className="w-7 h-7 text-gold" strokeWidth={1.5} />
      </div>
      ...
      >
        <span className="eyebrow block mb-4">ANCESTRAL JOURNEY</span>
        <h2 className="font-serif text-h2 text-text-primary mb-6">
          Find your village.<br />
          Meet your history.
        </h2>
        <p className="text-text-secondary leading-relaxed mb-8 max-w-[30vw] ml-auto">
          We trace land records, map family neighborhoods, and arrange local 
          meetings—so your trip becomes a homecoming.
        </p>
        <a
          href="#contact"
          className="btn-outline inline-flex items-center gap-2 ml-auto"
        >
          Start a roots search
...
export default AncestralJourneySection;